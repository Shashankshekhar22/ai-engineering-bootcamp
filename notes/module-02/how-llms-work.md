...
Training is training from huge data set, Training a model from the data which is not available.

inference is, Model is already trined and getting information from that

Training: model parameters/weights are updated based on data and loss.

Inference: the trained model's parameters are generally not updated; it generates output from the input/context.

6. What is a context window?

Context window is the the combination of Questions which were asked, any relevant information which is already avaiable to the model, System instructions, Retrieved Rag documents and Tool Results
...
RAG reduces certain hallucination

RAG retrieves relevant information from an external knowledge source and places it into the model's context so the answer can be grounded in relevant evidence.
